<template>
    <div class="text-xs-center">
        <v-card>
            <v-card-title class="title layout justify-center font-weight-light" primary-title>
                Asignar cita
            </v-card-title>
                        ...
                        <v-layout wrap>
                            ...
                            <v-flex xs5>
                                <v-text-field v-show="paciente.Ut == 'REDVITAL UT'" v-model="paciente.NombreIPS"
                                    readonly label="IPS"></v-text-field>
                            ...
                            <v-flex xs4>
                                <v-text-field v-model="paciente.Medico" readonly label="Medico Familia">
                                </v-text-field>
                            </v-flex>
                            <v-flex xs3>
                    ...
                    <v-layout row wrap>
                        <v-flex xs3 px-1>
                            ...
                            <v-autocomplete v-model="especialidad_selected" :items="especialidades" item-text="Nombre"
                                item-value="id" label="Especialidades"
                                @input="fetchSedes(), campoCup(), campoOrden(), campoTeleconsulta()">
                            </v-autocomplete>
                        </v-flex>
                        <v-flex xs9 px-1 v-if="campoTeleconsulta()">
                            <v-checkbox v-model="teleconsulta" @change="fetchAgendas()" label="Teleconsulta"
                                color="primary">
                            </v-checkbox>
                        </v-flex>
                        <v-flex xs2 px-1 v-show="campOrden">
                            <v-autocomplete v-model="data.orden_cita" :items="ordenCups" item-text="orden_id"
                                item-value="orden_id" label="Orden" @input="cupOrden(data.orden_cita)"></v-autocomplete>
                        </v-flex>
                        <v-flex xs7 px-1 v-show="campOrden">
                            <v-autocomplete v-model="data.cuporden" :items="cupsOrdencita" item-text="nombre"
                                item-value="cuporden_id" label="Cup"></v-autocomplete>
                        </v-flex>
                        <!-- sede -->
                        <v-flex xs4 px-1 v-if="teleconsulta == false">
                            <v-select v-model="sede_selected" :items="sedes" label="Sede" item-text="Nombre"
                                item-value="id" @input="fetchAgendas(), getSede()"></v-select>
                        </v-flex>
                        ...
                        <v-flex xs5 px-1 v-if="teleconsulta == false">
                            <v-autocomplete v-model="actividad_selected" :items="actividades" item-text="name"
                                item-value="et_id" label="Actividad" @input="fetchAgendas(), getTipoActividad()"></v-autocomplete>
                        </v-flex>
                                        ...
                                        <v-layout wrap>
                                            ...
                                            <v-flex xs12 sm3 md3>
                                                <v-text-field label="Número Doc" type="number"
                                                    v-model="formPaciente.Num_Doc" required>
                                                </v-text-field>
                                            </v-flex>
                                            <v-flex xs12 sm3 md3>
                                            ...
                                            <v-flex xs12 sm3 md3>
                                                <v-select :items="ocupacional" label="Entidad Salud"
                                                    v-model="formPaciente.entidad_id" item-text="nombre" item-value="id"
                                                    required>
                                                </v-select>
                                            </v-flex>
                                            <!-- <v-flex xs12 sm3 md3>
    ...
    export default {
        ...
        methods: {
            ...
            guardar_paciente() {
                ...
                axios.post('/api/paciente/create', this.formPaciente)
                    .then(res => {
                        if (!res.data.estado) {
            ...
            campoTeleconsulta() {
                if (this.especialidad_selected == 1) return true;
            },
            campoCup() {
            ...
            fetchAgendas() {
                ...
                if (this.teleconsulta == true) {
                    axios.post(`/api/redvital/agendaDisponible/516`, this.paciente)
                        .then((res) => {
            ...
            search_paciente() {
                if (!this.this.datosCita.cedula) return;

                this.fetchEspecialidades();
                axios.get(`/api/paciente/showEnabled/${this.datosCita.cedula}`)
                    .then((res) => {
            ...
            async citaOrdenPaciente() {
                this.ordenCups = []
                await axios.post(`/api/cita/citaOrdenPaciente/${this.paciente.id}`, {
                        especialidad: this.especialidad_selected
                    })
                    .then((res) => {
            ...
            async campoOrden() {
                ...
                await axios.get(`/api/especialidad/especialidadCita/${this.especialidad_selected}`)
                    .then((res) => {
            ...
            getTipoActividad() {
                const findActividad = this.actividades.find(a => a.et_id === this.actividad_selected);
