                    ...
                    <template v-if="data.Tiporden_id == 2 || data.Tiporden_id == 4">
                        <v-flex xs12>
                            <v-layout row wrap>
                                ...
                                <v-flex xs12>
                                    <v-autocomplete label="Paquete Servicios" item-text="nombre" :items="listaPaquetes" v-model="procedimiento.paquete" return-object></v-autocomplete>
                                </v-flex>
                                <v-flex xs2>
...
<script>
    ...
    export default {
        ...
        methods: {
            getNivel() {
                axios.get('/api/orden/getNivel').then(res => {
                    this.nivelUsuario = parseInt(res.data);
                })
            },
            ...
            getDetaServicios: async function (service) {
                ...
                let Ago = moment(this.procedimiento.date)
                    .subtract(days, "d")
                    .format("YYYY-MM-DD");
                let Next = moment(this.procedimiento.date)
                    .add(days, "d")
                    .format("YYYY-MM-DD");
                ...
                await axios
                    .post("/api/orden/getCupOrden", {
                        ...
                        Ago: Ago,
                        Next: Next,
                        fechaOrden: this.procedimiento.date
                    })
                    .then(res => {
                ...
                if (
                    ...
                    switch (this.data.Tiporden_id) {
                        ...
                        case 6:
                            ...
                            break;
                        case 9:
                            this.fetchRecomendacion.forEach((ser) => {
                                let pdf = {
            ...
            fetchRecomendacionOrden() {
                axios.get('/api/orden/fetchRecomendacion/' + this.cita_paciente).then((res) => {
                    this.fetchRecomendacion = res.data;
                });
            },
            generateRecomendacion() {
                if (this.recomendaciones.recomedacion == "" && this.recomendaciones.FechaFinal == "") {
                    swal({
                        title: "Recomendacion",
                ...
                swal({
                    ...
                    if (willDelete) {
                        ...
                        await axios.post(
                            `/api/orden/citapaciente/${this.cita_paciente}/create`, this
                            .data).then((res) => {
                            this.order_id = res.data.orden_id;
                            this.fetchRecomendacionOrden();
                            this.clearRecomendacion();
                            swal("¡Orden creada de manera exitosa!", {
                                timer: 2000,
                                icon: "success",
                                buttons: false,
            ...
            async agregarPaquete(){
                if (this.procedimiento.paquete.hasOwnProperty("id")) {
                    this.preload = true;
                    for (const s of this.procedimiento.paquete.detalle_paquete) {
                        let data = {
                            Cantidad:s.cantidad,
                            cup:s.cup
                        }
                        let service = await this.getDetaServicios(data);
                        if (service) {
                            this.preload = false;
                            let msg = `Existen Ordenes activas para ése procedimiento por la cantidad ${
                                service.Cantidad
                            ...
                            swal({
                                ...
                                if (willDelete) {
                                    this.pushProcedimientoPaquete(s,service.Cantidad);
                                }
                            });
                        } else {
                            this.pushProcedimientoPaquete(s);
                        }
                    }
                }
            },

            pushProcedimientoPaquete(item,cantidad = 0) {
                if (parseInt(item.cup.Cantidadmaxordenar) >= parseInt(item.cantidad) + parseInt(cantidad)) {
                    this.preload = true;
                    this.data.procedimientos.push({
                        id: item.cup.id,
                        codigo: item.cup.Codigo,
                        nombre: item.cup.Nombre,
                        auditoria: item.cup.Requiere_auditoria,
