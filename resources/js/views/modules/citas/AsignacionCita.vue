<template>
  <v-layout>
      ...
      <div class="text-xs-center">
        <v-dialog v-model="dialog" width="500">
          <v-card>
            ...
            <v-card-text v-if="teleconsulta == false">
              Asignar cita de tipo <b> {{ actividad }} </b> al usuario
              <b
                >{{ paciente.Primer_Nom }} {{ paciente.SegundoNom }}
                {{ paciente.Primer_Ape }} {{ paciente.Segundo_Ape }}</b
              >
              identificado con <b>{{ paciente.Tipo_Doc }}</b> N°
              <b>{{ paciente.Num_Doc }}</b> el día <b>{{ fecha_selected }}</b> a
              las <b>{{ data.hora_inicio | time }}</b> en la sede
              <b>{{ sede }}</b
              >, <b>{{ data.consultorio }}</b> con el médico
              <b>{{ medico_selected }}</b>
            </v-card-text>
            <v-card-text v-else>
              Asignar cita de tipo <b> {{ data.tipo }} </b> al usuario
              <b
                >{{ paciente.Primer_Nom }} {{ paciente.SegundoNom }}
                {{ paciente.Primer_Ape }} {{ paciente.Segundo_Ape }}</b
              >
    ...
    <v-flex shrink xs4 mr-1>
      ...
      <v-flex v-show="paciente.id">
        <v-expansion-panel>
              ...
              <div>
                <v-badge right>
                  <template v-slot:badge>
                    <span v-if="cita_pendiente.Tipo_agenda">{{
                      allcita_pendiente.length
                    }}</span>
                  </template>
                ...
                <v-flex xs12 v-if="cita_pendiente.Tipo_agenda">
                  <v-list two-line subheader>
                    ...
                    <p class="text-md-center">
                      {{ cita_pendiente.Especialidad }}
                    </p>
                    <v-list-tile avatar>
                      <v-list-tile-content>
                        <v-list-tile-title>Fecha</v-list-tile-title>
                        <v-list-tile-sub-title
                          >{{ cita_pendiente.Fecha | fecha_cita_pendiente }}
                        </v-list-tile-sub-title>
                      </v-list-tile-content>
                    </v-list-tile>
                    <v-list-tile avatar>
                    ...
                    <v-list-tile avatar>
                      <v-list-tile-content>
                        ...
                        <v-list-tile-sub-title
                          >{{ cita_pendiente.Nombre_medico }}
                          {{
                            cita_pendiente.Apellido_medico
                          }}</v-list-tile-sub-title
                        >
                      </v-list-tile-content>
                    </v-list-tile>
                  ...
                  <v-layout>
                    ...
                    <v-btn
                      ...
                      small
                      color="primary"
                      @click="todaspendientesmodal = true"
                      >ver más!
                    </v-btn>
                  </v-layout>
                  <v-dialog v-model="todaspendientesmodal" width="80%">
                    <v-card>
                      <v-card-title class="headlines lighten-2" primary-title
                        >Citas Pendientes
                      </v-card-title>
                      <v-card-text>
                        <v-data-table
                        ...
                        >
                          <template v-slot:items="props">
                            ...
                            <td class="text-xs-center">
                              {{ props.item.Tipo_agenda }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.Fecha | fecha_cita_pendiente }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.Hora_Inicio | hora_cita_pendiente }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.Sede }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.Nombre_medico }}
                              {{ props.item.Apellido_medico }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.User_asgina }}
                              {{ props.item.ApUser_asgina }}
                            </td>
                            <td class="text-xs-center">
          ...
          <v-expansion-panel-content v-show="paciente.id">
            <template v-slot:header>
              <div>
                <v-badge right>
                  <template v-slot:badge>
                    <span v-if="cita_anteriores.Tipo_agenda">{{
                      allcita_anteriores.length
                    }}</span>
                  </template>
                  <span>Citas Atendias, Inasistidas y Canceladas</span>
                </v-badge>
              </div>
            </template>
            <v-divider></v-divider>
            <v-card v-show="paciente.id">
              <v-card-text>
                <v-flex xs12 v-if="cita_anteriores.Tipo_agenda">
                  <v-list two-line subheader>
                    ...
                    <v-list-tile avatar>
                      <v-list-tile-content>
                        <v-list-tile-title>Médico</v-list-tile-title>
                        <v-list-tile-sub-title
                          >{{ cita_anteriores.Nombre_medico }}
                          {{
                            cita_anteriores.Apellido_medico
                          }}</v-list-tile-sub-title
                        >
                      </v-list-tile-content>
                    </v-list-tile>
                  ...
                  <v-layout>
                    ...
                    <v-btn
                      ...
                      small
                      color="primary"
                      @click="todasanterioresmodal = true"
                      >ver más!
                    </v-btn>
                  </v-layout>
                  <v-dialog v-model="todasanterioresmodal" width="80%">
                    <v-card>
                      ...
                      <v-card-text>
                        <v-data-table
                          :headers="headercitaanteriores"
                          :items="allcita_anteriores"
                          class="elevation-1"
                          hide-actions
                        >
                          <template v-slot:items="props">
                            ...
                            <td class="text-xs-center">
                              {{ props.item.Fecha | fecha_cita_anteriores }}
                            </td>
                            <td class="text-xs-center">
                              {{
                                props.item.Hora_Inicio | hora_cita_anteriores
                              }}
                            </td>
                            <td class="text-xs-center">
                              {{ props.item.Sede }}
                            ...
                            <td
                              ...
                              v-if="
                                props.item.User_asgina &&
                                props.item.Nom_estado !== 'Atendida'
                              "
                            >
                              {{ props.item.User_asgina }}
                              {{ props.item.ApUser_asgina }}
                            ...
                            <td
                              class="text-xs-center"
                              v-else-if="props.item.Nom_estado === 'Atendida'"
                            >
                              {{ props.item.User_asignaAtendido }}
                              {{ props.item.ApUser_asignaAtendido }}
                            </td>
                            <td class="text-xs-center" v-else>Paciente</td>
                            <td class="text-xs-center">
                              <v-chip
                                v-if="props.item.Nom_estado === 'Atendida'"
                                class="ma-2"
                                color="green"
                                text-color="white"
                                @click="showOrdenes(props.item)"
                              >
                                {{ props.item.Nom_estado }}
                              </v-chip>
                              <v-chip
                                v-else-if="
                                  props.item.Nom_estado === 'Cancelada'
                                "
                                class="ma-2"
                                color="red"
                                text-color="white"
                              ...
                              <v-chip
                                v-else-if="
                                  props.item.Nom_estado === 'Inasistencia'
                                "
                                class="ma-2"
                                color="orange"
                                text-color="white"
                      ...
                      <v-card-actions>
                        ...
                        <v-btn
                          color="primary"
                          flat
                          @click="todasanterioresmodal = false"
                          >Cerrar
                        </v-btn>
                      </v-card-actions>
                    </v-card>
        ...
        <v-btn
          ...
          color="info"
          block
          @click="(dialoglistadoDemanda = true), listarDemanda()"
        >
          <v-badge color="red" right>
            <template v-slot:badge>
              <span>{{ allDemanda.length }}</span>
            </template>
            <span>Demanda Instatisfecha</span>
          </v-badge>
        </v-btn>
      </v-flex>

      <v-dialog v-model="dialoglistadoDemanda" width="80%">
        <v-card>
          <v-card-title class="headline success" style="color: white">
            <span class="title layout justify-center font-weight-light"
              >Agregar Demanda Insatisfecha</span
            >
          </v-card-title>
          <v-card-text>
            <v-data-table
              :headers="headerDemanda"
              :items="allDemanda"
              class="elevation-1"
              hide-actions
            >
              <template v-slot:items="props">
                ...
                <td v-if="can('cita.cerrarDemanda')">
                  <v-btn
                    dark
                    color="info"
                    @click="modalCerrarDemanda(props.item)"
                    >Cerrar</v-btn
                  >
                </td>
              </template>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" dark @click="dialoglistadoDemanda = false"
              >Cerrar
            </v-btn>
          </v-card-actions>
        </v-card>
      ...
      <v-dialog v-model="dialogCerrarDemanda" max-width="500px">
        <v-card>
          <v-card-title class="headline success" style="color: white">
            <span class="title layout justify-center font-weight-light"
              >Cerrar Demanda Insatisfecha</span
            >
          </v-card-title>
          <v-card-text>
            <v-flex xs12>
              <v-text-field
                type="date"
                v-model="cerrarDemanda.fecha_cita"
                label="Fecha de la cita"
              >
              </v-text-field>
              <v-text-field
                v-model="cerrarDemanda.consultorio"
                label="Consultorio o Sala"
              ></v-text-field>
            </v-flex>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" @click="cerrarModalDemanda()" round
              >Cerrar</v-btn
            >
            <v-btn color="info" @click="guardarCierreDemanda()" round
              >Guardar</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-dialog
        v-model="dialogshowMotivo"
        v-if="dialogshowMotivo"
        max-width="400px"
      >
        <v-card>
          <v-card-title class="headline success" style="color: white">
            <span class="title layout justify-center font-weight-light"
              >Detalle de la Cita</span
            >
          </v-card-title>
          <v-card-text>
            <div>
              <span
                ><strong>Motivo:</strong> {{ descripcionMotivo.Motivo }}</span
              >
            </div>
            <span v-if="descripcionMotivo.fechaCancelo"
      ...
      <v-dialog
        v-model="dialogshowOrdenes"
        v-if="dialogshowOrdenes"
        max-width="400px"
      >
        <v-card>
          <v-card-title class="headline success" style="color: white">
          ...
          <v-card-text>
            <div>
              <span v-if="descripcionMotivo.ordens"
                ><strong>Motivo:</strong> Se usó la orden #
                {{ descripcionMotivo.ordens }} con el cups
                {{ descripcionMotivo.codigo_cups }} -
                {{ descripcionMotivo.nombre_cups }}</span
              ><span v-else
                ><strong>Motivo:</strong> la cita no solicitó número de
                orden</span
              >
            </div>
            <span v-if="descripcionMotivo.fecha_usada"
              ><strong>Fecha y Hora:</strong>
              {{ descripcionMotivo.fecha_usada.split(".")[0] }}</span
            >
          </v-card-text>
        </v-card>
      </v-dialog>

      <v-flex mt-3 v-show="paciente.id">
        <v-card>
          <v-card-title>
            <span class="title layout justify-center font-weight-light"
    ...
    <v-flex shrink xs8 ml-1 v-show="paciente.id">
      <v-card max-height="100%" class="mb-3">
        <v-card-title class="headline success" style="color: white">
          <span class="title layout justify-center font-weight-light"
            >Datos Paciente</span
      ...
      <v-card max-height="100%" v-show="true">
        ...
        <v-card-text>
          <v-layout row wrap>
            <v-flex xs3 px-1>
              ...
              <v-autocomplete
                ...
                @input="campoCup(), campoOrden(), campoTeleconsulta()"
              >
              </v-autocomplete>
            </v-flex>
            <v-flex xs9 px-1 v-if="campoTeleconsulta()">
              <v-checkbox
                v-model="teleconsulta"
                @change="fetchAgendas()"
                label="Teleconsulta"
                color="primary"
              >
            ...
            <v-flex xs2 px-1 v-show="campOrden">
              <v-autocomplete
                v-model="data.orden_cita"
                :items="ordenCups"
                item-text="orden_id"
                item-value="orden_id"
                label="Orden"
                @input="cupOrden(data.orden_cita)"
              ></v-autocomplete>
            </v-flex>

            <v-flex xs6 px-1 v-show="campOrden">
              <v-autocomplete
                v-model="data.orden_cita"
                :items="ordenCups"
                item-text="Observacionorden"
                item-value="orden_id"
                label="Observacion de la orden"
                @input="cupOrden(data.orden_cita)"
                readonly
              ></v-autocomplete>
            </v-flex>

            <v-flex xs8 px-1 v-show="campOrden">
              <v-autocomplete
                v-model="data.cuporden"
                :items="cupsOrdencita"
                item-text="nombre"
                item-value="cuporden_id"
                label="Cup"
              ></v-autocomplete>
            </v-flex>
            <!-- sede -->
            <v-flex xs5 px-1 v-if="teleconsulta == false">
              <v-autocomplete
                ...
                item-value="et_id"
                label="Actividad"
                @input="fetchAgendas(), getTipoActividad(), fetchSedes()"
              >
              </v-autocomplete>
            </v-flex>
            <v-flex xs6 px-1 v-if="teleconsulta == false">
              <v-autocomplete
            ...
            <v-flex xs3 px-1 v-if="teleconsulta == false">
              <v-autocomplete
            ...
            <v-flex
              ...
              px-1
              v-if="
                data.tipo_paciente == 'Hospitalario' ||
                data.tipo_paciente == 'Urgencias'
              "
            >
              <v-text-field
                label="Ubicación"
            ...
            <v-flex
              xs9
              px-2
              v-if="(especialidad_selected != null, actividad_selected != null)"
            >
              <v-btn dark color="info" @click="abrirDemanda()"
                >Demanda Insatisfecha</v-btn
              >
            </v-flex>
          </v-layout>

          <v-dialog v-model="dialogDemanda" width="500" persistent>
            <v-card>
              <v-card-title class="headline success" style="color: white">
                <span class="title layout justify-center font-weight-light"
                  >Agregar Demanda Insatisfecha</span
              ...
              <v-card-text>
                <v-flex xs12 px-2>
                  <v-textarea label="Observación" v-model="demanda.observacion">
                  </v-textarea>
                </v-flex>
              </v-card-text>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="error" dark @click="dialogDemanda = false">
                  Cerrar
                </v-btn>
                <v-btn color="primary" dark @click="guardarDemanda()">
                  Guardar
                </v-btn>
              </v-card-actions>
            </v-card>
                  ...
                  <v-form @submit.prevent="guardar_paciente()">
                    <v-layout wrap>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Segundo Nombre"
                          v-model="formPaciente.SegundoNom"
                          required
                        >
                        </v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Primer Apellido"
                          v-model="formPaciente.Primer_Ape"
                          required
                        >
                        </v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Segundo Apellido"
                          v-model="formPaciente.Segundo_Ape"
                          required
                        ></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Fecha Nacimiento"
                          v-model="formPaciente.Fecha_Naci"
                          required
                        >
                        </v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Discapacidad"
                          v-model="formPaciente.Discapacidad"
                          required
                        ></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          ...
                          v-model="formPaciente.Grado_Discapacidad"
                          required
                        ></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="['BENEFICIARIO', 'COTIZANTE', 'SUBSIDIADO']"
                          label="Tipo Afiliado"
                          required
                          v-model="formPaciente.Tipo_Afiliado"
                        >
                        </v-select>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-autocomplete
                          ...
                          hide-details
                          v-model="formPaciente.Departamento"
                        ></v-autocomplete>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm6 md6>
                        <v-autocomplete
                          append-icon="search"
                          label="IPS atención..."
                          :items="sedesprestadores"
                          item-text="Nombre"
                          item-value="id"
                          hide-details
                          v-model="formPaciente.IPS"
                        ></v-autocomplete>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-autocomplete
                          append-icon="search"
                          label="Mpio donde Labora"
                          :items="municipios"
                          item-text="municipio"
                          item-value="id"
                          hide-details
                          v-model="formPaciente.Mpio_Labora"
                        ></v-autocomplete>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Direccion Residencia"
                          v-model="formPaciente.Direccion_Residencia"
                          required
                        ></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-autocomplete
                          ...
                          item-value="id"
                          hide-details
                          v-model="formPaciente.Mpio_Residencia"
                        >
                        </v-autocomplete>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          ...
                          v-model="formPaciente.Sexo_Biologico"
                          required
                        ></v-text-field>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="nhijos"
                          label="Num Hijos"
                          v-model="formPaciente.Num_Hijos"
                          required
                        >
                        </v-select>
                      </v-flex>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          ...
                          v-model="formPaciente.Nivelestudio"
                          required
                        ></v-text-field>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          ...
                          v-model="formPaciente.Tipovinculacion"
                          required
                        ></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-text-field
                          label="Ocupacion"
                          v-model="formPaciente.Ocupacion"
                          required
                        >
                        </v-text-field>
                      </v-flex>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="ocupacional"
                          label="Entidad Salud"
                          v-model="formPaciente.entidad_id"
                          item-text="nombre"
                          item-value="id"
                          required
                        >
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="estadoCivil"
                          label="Estado civil"
                          v-model="formPaciente.estado_civil"
                          required
                        ></v-select>
                      </v-flex>
                      <!-- <v-flex xs12 sm3 md3>
                      ...
                      <v-flex xs12 sm3 md3>
                        <v-select
                          ...
                          label="cargo laboral"
                          required
                          v-model="formPaciente.cargo_laboral"
                        >
                        </v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="['0', '1 a 2', '3 a 4', '> 4']"
                          label="composicion familiar"
                          required
                          v-model="formPaciente.composicion_familiar"
                        ></v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          ...
                          label="vivienda"
                          required
                          v-model="formPaciente.vivienda"
                        >
                        </v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="['Ninguna', '(1-3)', '(4-6)', 'Mas de  6']"
                          label="personas a cargo"
                          required
                          v-model="formPaciente.personas_a_cargo"
                        ></v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          ...
                          required
                          v-model="formPaciente.tipo_contratacion"
                        >
                        </v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          ...
                          label="antiguedad en empresa"
                          required
                          v-model="formPaciente.antiguedad_en_empresa"
                        ></v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          ...
                          label="antiguedad cargo actual"
                          required
                          v-model="formPaciente.antiguedad_cargo_actual"
                        ></v-select>
                      </v-flex>
                      <v-flex xs12 sm3 md3>
                        <v-select
                          :items="['Ninguno ', '(1 - 3)', '(4 -6) ', '> 6']"
                          label="# cursos a cargo"
                          required
                          v-model="formPaciente.numero_cursos_a_cargo"
                        ></v-select>
                      </v-flex>
                    </v-layout>
                  </v-form>
          ...
          <!-- <v-dialog v-model="creaPaciente" persistent max-width="500">
                        <v-card>
                            <v-card-title class="headline info" style="color:white">
                                <span class="title layout justify-center font-weight-light">Creacción de nuevo
                                    paciente</span>
                            </v-card-title>
                            <v-card-text style="color:red"><b>El paciente con la cédula ingresada no se encuentra
                                    registrado!</b></v-card-text>
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn v-if="can('crear.pacientes')" color="success" dark
                                    @click="pacienteCreacion = true,  creaPaciente = false">Crear paciente</v-btn>
                                <v-btn color="red" dark @click="creaPaciente = false">Cerrar</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-dialog> -->
          <v-container grid-list-md fluid class="pa-0">
...
export default {
  ...
  data() {
    return {
      ...
      headercitaanteriores: [
        ...
        {
          text: "Usuario que Inasiste, cancela o asigna",
          align: "center",
          sortable: false,
          value: "User_asgina",
        },
  ...
  computed: {
    ...mapGetters(["can"]),
    agendaDisponible() {
      if (this.teleconsulta == true) {
        let citasEnable = [];
        let citas = [];
        for (let i = 0; i < this.agendas.length; i++) {
          let citas = [];
          if (this.agendas[i].fecha == this.fecha) {
        ...
        return citasEnable.sort(
          (a, b) => moment(a.hora_inicio) - moment(b.hora_inicio)
        );
      } else {
        let citasEnable = [];
        let citas = [];
    ...
    medicosSede() {
      return this.agendas.map(
        (agenda) => `${agenda.nombreMedico} ${agenda.apellidoMedico}`
      );
    },
    especialiadesq() {
    ...
    actividades() {
      return this.especialidades.filter(
        (e) => this.especialidad_selected === e.id
      );
    },
  },
  mounted() {
  ...
  methods: {
    fetchEntidades() {
      axios.get("/api/entidades/entidadesOcpucacionales").then((res) => {
    ...
    guardar_paciente() {
      ...
      axios.post("/api/paciente/create", this.formPaciente).then((res) => {
        if (!res.data.estado) {
    ...
    fetchEspecialidades() {
      axios.get(`/api/agenda/agendaDisponible/especialidades`).then((res) => {
        const response = res.data;
        if (this.logueado.id == 1276 || this.logueado.id == 1802) {
          this.especialidades = response.filter(
            (item) =>
              item.Nombre == "Optometria" || item.Nombre == "Oftalmologia"
          );
        } else {
    ...
    campoTeleconsulta() {
      if (this.especialidad_selected == 1) {
        if (this.paciente.Correo1 === null) {
          swal("El correo electronico es requerido");
          return;
        }
        return true;
      }
    ...
    fetchSedes() {
      this.sedes = [];
      axios
        .post(`/api/agenda/agendaDisponible/sedes`, {
          especialidad: this.especialidad_selected,
        })
        .then((res) => {
          if (
            parseInt(this.paciente.entidad_id) !== 7 &&
            parseInt(this.actividad_selected) === 1 &&
            parseInt(this.especialidad_selected) === 1 &&
            parseInt(this.paciente.IPS) !== 184
          ) {
            const s = res.data.filter(
              (obj) =>
                parseInt(obj.Sedeprestador_id) === parseInt(this.paciente.IPS)
            );
            if (s.length > 0) {
              this.sedes = s;
            } else {
    ...
    fetchAgendas() {
      ...
      if (this.teleconsulta == true) {
        axios
          .post(`/api/redvital/agendaDisponible/516`, this.paciente)
          .then((res) => {
        ...
        if (
          ...
          this.sede_selected &&
          this.actividad_selected
        ) {
          axios
            .post("/api/agenda/agendaDisponible", {
              fecha: this.fecha,
    ...
    cupsPaciente(Paciente_id) {
      axios.get(`/api/cita/cupspaciente/${Paciente_id}`).then((res) => {
    ...
    async citaOrdenPaciente() {
      this.preload_cita = true;
      this.ordenCups = [];
      await axios
        .post(`/api/cita/citaOrdenPaciente/${this.paciente.id}`, {
          especialidad: this.especialidad_selected,
        })
        .then((res) => {
          this.ordenCups = res.data.map((cupor) => {
            return {
              cup_id: cupor.id,
              nombre: cupor.Nombre,
              orden_id: cupor.Orden_id,
              cuporden_id: cupor.cuporden_id,
              Observacionorden: cupor.Observacionorden,
            };
          });
    ...
    async campoOrden() {
      if (this.especialidad_selected == null) return;
      this.campOrden = false;
      this.cupsOrdencita = [];
      this.data.cuporden = "";
      await this.citaOrdenPaciente();
      await axios
        .get(`/api/especialidad/especialidadCita/${this.especialidad_selected}`)
        .then((res) => {
          this.necesitaOrden = res.data;
          if (this.necesitaOrden == true && this.ordenCups.length == 0) {
            swal({
              title: "¡Información!",
              text: "No se puede asignar cita de la especialidad sin una Orden",
              timer: 4000,
              icon: "info",
              buttons: false,
            });
          } else if (this.necesitaOrden == true) {
            this.campOrden = true;
          }
        });
    },
    cupOrden(orden) {
      this.cupsOrdencita = [];
      this.ordenCups.forEach((element) => {
        if (element.orden_id == orden) {
          this.cupsOrdencita.push(element);
        }
      });
    },
    asignarCita(cita) {
      if (this.teleconsulta == true) {
        this.data.medico = cita.medico;
        this.data.tipo = "Teleconsulta";
      }
      if (this.data.fecha_solicitada == null) {
        this.$alerError("Debe ingresar la fecha solicitada");
        return;
      }
      this.dialog = true;
    ...
    cancelarCita() {
      ...
      axios
        .put(`/api/cita/cancelar/${this.cancelar.id}`, this.cancelar)
        .then((res) => {
          swal({
    ...
    confirmarCitaModal(cita) {
      ...
      swal({
        ...
        if (confirm) {
          axios
            .put(`/api/cita/confirmar/${this.cita.id}`, this.confirmar)
            .then((res) => {
              swal({
    ...
    confirmarCita() {
      ...
      swal({
        ...
        if (confirm) {
          axios
            .put(
              `/api/cita/confirmar/${this.cita_pendiente.id}`,
              this.confirmar
            )
            .then((res) => {
              swal({
    ...
    agendarCita() {
      if (this.necesitaOrden == true) {
        if (this.data.cuporden == undefined || this.data.cuporden == "") {
          this.$alerError("No puede asignar la cita sin una Orden y Cup!");
          return;
        }
      }
      ...
      this.preload_cita = true;
      axios
        .put(`/api/cita/asignarcita/${this.data.cita_id}`, this.data)
        .then((res) => {
          this.dialog = false;
          this.clearFields();
          this.fetchAgendas();
        ...
        if (agenda.fecha == val) {
          ...
          if (this.teleconsulta == true) {
            if (agenda.citas.length > 0) {
              if (agenda.citas[0].Hora_Inicio.substring(0, 10) === dia)
                many = true;
            }
          } else {
    ...
    showOrdenes(ordenes) {
      this.dialogshowOrdenes = true;
      this.descripcionMotivo = ordenes;
    },
    clearCrearPaciente() {
    ...
    getTipoActividad() {
      const findActividad = this.actividades.find(
        (a) => a.et_id === this.actividad_selected
      );
      this.actividad = findActividad.name;
    },
    getSede() {
      const findSede = this.sedes.find((a) => a.id === this.sede_selected);
      this.sede = findSede.Nombre;
    },
    ...
    abrirDemanda() {
      ...
      this.demanda.especialidad_id = this.especialidad_selected;
      this.demanda.especialidad_tipo_agenda_id = parseInt(
        this.actividad_selected
      );
    },

    guardarDemanda() {
      if (!this.demanda.observacion) {
        this.$alerError("Debe agregar una observacion");
        return;
      }
      this.demanda.paciente_id = this.paciente.id;
      this.preload_cita = true;
      axios
        .post(`/api/cita/agregarDemanda`, this.demanda)
        .then((res) => {
    ...
    listarDemanda() {
      axios
        .get("/api/cita/listarDemanda/" + this.paciente.id)
        .then((res) => {
          this.allDemanda = res.data;
        })
        .catch((err) => this.showMessage(err.response.data.message));
    },

    modalCerrarDemanda(item) {
      this.dialogCerrarDemanda = true;
      this.cerrarDemanda = item;
    },

    cerrarModalDemanda() {
      this.dialogCerrarDemanda = false;
      (this.cerrarDemanda.fecha_cita = null),
        (this.cerrarDemanda.consultorio = null);
    },

    guardarCierreDemanda() {
      if (!this.cerrarDemanda.fecha_cita) {
        this.$alerError("Debe agregar una fecha");

        return;
      }
      if (!this.cerrarDemanda.consultorio) {
        this.$alerError("Debe agregar un consultorio o sala");
        return;
      }

      this.preload_cita = true;
      axios
        .post(`/api/cita/agregarCierreDemanda`, this.cerrarDemanda)
        .then((res) => {
          this.dialogCerrarDemanda = false;
          this.preload_cita = false;
          this.listarDemanda();
