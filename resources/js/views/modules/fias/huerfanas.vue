                            ...
                            <v-layout row wrap>
                                <v-flex xs12 sm12 md12>
                                    ...
                                    <v-card>
                                        ...
                                        <v-data-table :headers="headers" :items="huerfanasData" :search="search">
                                            <template v-slot:items="props">
                                                <td>{{props.item.Num_Doc}}</td>
                                                <td>{{props.item.Nombre}}</td>
                                                <td>{{props.item.patologia}}</td>
                                                <td>{{props.item.tipo_de_tratamiento}}</td>
                                                <td>{{props.item.fecha_de_diagnóstico}}</td>
                                                <td>{{props.item.fecha_de_ingreso_al_programa}}</td>
                                                <td>{{props.item.fecha_inicio_tratamiento}}
                                                <td>{{props.item.fecha_ultima_atención}}
                                                <td>{{props.item.fecha_ultimo_tratamiento}}
                                                </td>
                                                <td>
                                                    <v-tooltip top>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [{
                    ...
                    {
                        text: 'F.ingreso programa'
                    },
                    {
                        text: 'F.inicio tratamiento'
                    },
                    {
                        text: 'F.ultima atención'
                    },
                    {
                        text: 'F.ultimo tratamiento'
                    },
                ],
                huerfanasData: [],
                referencias: [{
        ...
        methods: {
            fiasHuerfanas() {
                this.preload = true;
                axios.post('/api/fias/allHuerfanas', this.consultaFiltrada)
                    .then(res => {
                        console.log('c r', res.data)
                        this.huerfanasData = res.data;
                        this.preload = false;
                    }).catch(e => {
                        console.log(e);
                        this.preload = false;
            ...
            consultar() {
                axios.post('/api/fias/consulthuerfanas', this.consultaFiltrada).then(res => {
                    this.referencias[0].title = `${res.data.huerfanas}`;
                });
                this.fiasHuerfanas()
            },
            sedes() {
