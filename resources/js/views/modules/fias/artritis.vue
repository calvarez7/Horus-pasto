                            ...
                            <v-layout row wrap>
                                <v-flex xs12 sm12 md12>
                                    ...
                                    <v-card>
                                        ...
                                        <v-data-table :headers="headers" :items="artritisData" :search="search">
                                            <template v-slot:items="props">
                                                <td>{{props.item.Num_Doc}}</td>
                                                <td>{{props.item.Nombre}}</td>
                                                <td>{{props.item.descripcion_Cie_10}}</td>
                                                ...
                                                <td>{{props.item.fecha_ultima_atencion}}
                                                <td>{{props.item.fecha_ultimo_tratamiento}}
                                                <td>{{props.item.tratamiento_actual}}
                                                </td>
                                                <td>
                                                    <v-tooltip top>
...
<script>
    ...
    export default {
        ...
        methods: {
            fiasArtritis() {
                this.preload = true;
                axios.post('/api/fias/allArtritis', this.consultaFiltrada)
                    .then(res => {
                        this.artritisData = res.data;
                        this.preload = false;
                    }).catch(e => {
                        console.log(e);
                        this.preload = false;
            ...
            consultar() {
                axios.post('/api/fias/consultartritis', this.consultaFiltrada).then(res => {
                    this.referencias[0].title = `${res.data.artritis}`;
                });
                this.fiasArtritis()
            },
            sedes() {
