    ...
    <v-flex xs6 px-1>
        <v-card>
            <v-container pa-1>
                ...
                <v-card-title>
                    <v-btn v-if="can('tipo-movimiento.create')" round @click="createTipo()" color="primary" dark ><v-icon left>person_add</v-icon>Crear Tipo</v-btn>
                    <v-spacer></v-spacer>
                    <v-flex 
                        sm5
                        xs12
                ...
                <v-data-table
                    ...
                    <template v-slot:items="props">
                        ...
                        <td class="text-xs-right">
                            <v-btn v-if="can('tipo-movimiento.edit')" fab outline color="warning" small @click="editTipo(props.item)"><v-icon>edit</v-icon></v-btn>
                        </td>
                    </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            saveTipo(){
                axios.post('/api/tipo/create',this.data)
                .then(res => {
                    this.emptyData();
                    this.dialog = false;
                    this.fetchTipos();
            ...
            updateTipo() {
                axios.put(`/api/tipo/edit/${this.data.id}`,this.data)
                    .then(res => {   
                        this.emptyData();
                        this.dialog = false;
                        this.fetchTipos();
