    ...
    <v-layout row wrap>
        <v-flex xs12>
            <v-stepper v-model="e6" vertical>
                ...
                <v-stepper-step :complete="e6 > 3" editable :edit-icon="$vuetify.icons.complete" step="3">NOTAS ENFERMERIA
                </v-stepper-step>
                        ...
                        <v-layout row wrap>
                            <v-flex xs6>
                                <v-textarea
                                    ...
                                    name="input-7-4"
                                    label="Nota"
                                    v-model="formNotaEnfermeria.nota"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    ...
                                    v-model="formNotaEnfermeria.signos_alarma"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    ...
                                    v-model="formNotaEnfermeria.cuidados_casa"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    outline
                                    name="input-7-4"
                                    label="Casos de urgencia"
                                    v-model="formNotaEnfermeria.caso_urgencia"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    ...
                                    name="input-7-4"
                                    label="Alimentación"
                                    v-model="formNotaEnfermeria.alimentacion"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    ...
                                    v-model="formNotaEnfermeria.efectos_secundarios"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    outline
                                    name="input-7-4"
                                    label="Habitos de higiene"
                                    v-model="formNotaEnfermeria.habito_higiene"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    outline
                                    name="input-7-4"
                                    label="Derechso y deberes"
                                    v-model="formNotaEnfermeria.derechos_deberes"
                                ></v-textarea>
                            </v-flex>
                            <v-flex xs6>
                                <v-textarea
                                    outline
                                    name="input-7-4"
                                    label="Normas de la sala de quimioterapia"
                                    v-model="formNotaEnfermeria.normas_sala_quimioterapia"
                                ></v-textarea>
                            </v-flex>
                        </v-layout>
                    </v-container>
    ...
    export default {
        ...
        methods: {
         ...
         async  calcularIMC() {
            ...
            guardarExamenFisico() {
                this.getPaciente();
                this.formNotaEnfermeria.cita_paciente_id = this.citaPaciente;
                EventBus.$emit("change_disabled_list_history", "EXAMEN FÍSICO");
                this.preload_examensistema = true,
                axios
                    .post(
                    ...
                    .then(res => {
                        let url = '/api/esquemas/saveNote';
                        if(this.formNotaEnfermeria.id !== 0){
                            delete this.formNotaEnfermeria.id;
                            url = '/api/esquemas/editNote/'+this.citaPaciente;
                        }
                        axios.post(url,this.formNotaEnfermeria).then(res => {
            ...
            fetchExamen() {
                axios.get('/api/examenfisico/' + this.citaPaciente + '/getExamenFisico')
                    .then(res => {
                        ...
                        if(res.data.nota){
                            this.formNotaEnfermeria = res.data.nota;
                        }
                    });
            },
            clearFormNote() {
