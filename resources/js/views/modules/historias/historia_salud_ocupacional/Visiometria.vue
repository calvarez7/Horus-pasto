        ...
        <v-flex xs12>
            <v-stepper v-model="e6" vertical>
                ...
                <v-stepper-content step="6">
                    ...
                    <v-btn color="primary" round @click="saveEstereopsis()">Guardar y seguir</v-btn>
                </v-stepper-content>

                <v-stepper-step :complete="e6 > 7" editable :edit-icon="$vuetify.icons.complete" step="7">
                    TEST DE VISIÓN AL COLOR
                </v-stepper-step>
                <v-stepper-content step="7">
                    ...
                    <v-btn color="primary" round @click="saveVisioncolor()">Guardar y seguir</v-btn>
                </v-stepper-content>
                ...
                <v-stepper-content step="8">
                    ...
                    <v-btn color="primary" round @click="saveRetinoscopia()">Guardar y seguir</v-btn>
                </v-stepper-content>
                ...
                <v-stepper-step :complete="e6 > 12" editable :edit-icon="$vuetify.icons.complete" step="12">
                    CONCEPTO DE APTITUD LABORAL – VISIOMETRIA
                </v-stepper-step>
                <v-stepper-content step="12">
                    <v-card color="lighten-1" class="mb-5" height="auto">
                        <v-layout row wrap>
                            <v-flex xs6 sm8 class="px-5">
                                <v-autocomplete :items="aptitud"
                                    v-model="saludVisiometriaOcupacional.aptitud_laboral_visiomretria"
                                    label="CONCEPTO DE APTITUD LABORAL – VISIOMETRIA">
                                </v-autocomplete>
                            </v-flex>
                        </v-layout>
                    </v-card>
                    <v-btn color="primary" round @click="aptituplaboral()">Guardar y terminar</v-btn>
                </v-stepper-content>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                motivos: ['Examenes ocupacionales periódicos', 'Examenes ocupacionales ingreso',
                    ...
                    'Examenes ocupacionales reubicación'
                ],
        ...
        methods: {
            ...
            fetchAntecedentes() {
                axios.get("/api/antecedente/all").then(res => {
            ...
            getLocalStorage() {
                ...
                let visiometria = JSON.parse(localStorage.getItem("visiometria"));
                if (visiometria) {
            ...
            removeLocalStorage() {
                ...
                localStorage.removeItem("visiometria");
            },
            ...
            saveVisiometria() {
                axios.post('/api/cita/saveSaludocupacional/' + this.citaPaciente, this.saludVisiometriaOcupacional).then((res) => {
                    const Toast = Swal.mixin({
                        toast: true,
                        position: 'center-end',
                        background: '#4caf50',
