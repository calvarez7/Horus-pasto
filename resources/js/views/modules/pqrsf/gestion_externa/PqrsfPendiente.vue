<template>
    ...
    <div>
        <v-card>
            <v-card-title>
                ...
                <v-combobox :items="['Web', 'Supersalud']" label="Canal" single-line hide-details v-model="canal"
                    @input="getSlopeFilter()"></v-combobox>
                <v-tooltip bottom v-if="can('pqrsf.import')">
                    <template v-slot:activator="{ on }">
                        <v-btn v-on="on" color="success" fab small dark @click="importFile = true, getUser()">
                            <v-icon>add</v-icon>
                        </v-btn>
                    </template>
                    <span>Importar Pqrsf</span>
                </v-tooltip>
                <v-spacer></v-spacer>
                        ...
                        <v-layout wrap>
                            <v-flex xs12>
                                <v-layout row wrap>
                                    ...
                                    <v-flex xs12 v-for="(GesPqr, index) in pqrsf.gestion_pqrsfs" :key="`res-${index}`">
                                        ...
                                        <v-flex xs12 v-if="pqrsf.Pqr_codigo !== null">
                                            <span> <strong>Radicado Supersalud:</strong> {{pqrsf.Pqr_codigo}}</span>
                                        </v-flex>
                                        <v-flex>
        ...
        <v-dialog v-model="importFile" persistent max-width="500px">
            <v-card>
                <v-card-title>
                ...
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout wrap align-center>
                            ...
                            <v-flex xs12 sm12 md12>
                                <v-text-field readonly label="Usuario_id" v-model="usuario"></v-text-field>
                            </v-flex>
                            <v-flex xs12>
                                <v-img src="/images/Excel_Pqrsf.png"></v-img>
                            </v-flex>
                            <v-flex xs3 sm3 md3>
                                <v-btn color="success" @click="importArchive()">
                ...
                <template v-for="error in errores">
                    <v-flex xs12 px-12>
                        <v-alert :value="true" type="error">
                            El documento "{{error.Documento}}" no registra en pacientes.
                        </v-alert>
                    </v-flex>
                </template>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="error darken-1" flat @click="importFile = !importFile">Cancelar</v-btn>
                    <v-btn color="success darken-1" flat @click="importPqrsf()">Guardar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            getSlopeFilter() {
                ...
                axios.post('/api/pqrsf/slope', this.data)
                    .then(res => {
                        this.loading = false
            ...
            importPqrsf() {
                if (this.file.length <= 0) {
                    swal({
                        title: 'Error',
                        text: "Debe seleccionar el Excel a importar.",
                        type: "error",
                        icon: "error",
                    });
                    return;
                ...
                axios.post('/api/pqrsf/import',
                    ...
                    this.preload = false
                    this.$alerSuccess('Pqrsf masivas cargadas con exito!');
                    this.clearFile();
                }).catch((error) => {
                    this.preload = false
                    this.errores = error.response.data[0]
