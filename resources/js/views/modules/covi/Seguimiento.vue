            ...
            <v-card>
                ...
                <v-data-table :search="search" :headers="headers" :items="listaSolicitudes">
                    <template v-slot:items="props">
                        ...
                        <td>
                            <v-chip text-color="black">{{ props.item.destino }}</v-chip>
                        </td>
                        <td class="text-xs-left">
                            <v-btn round color="success" @click="imprimir(props.item.registro_id,'ficha')">Ficha</v-btn>
                        </td>
                        <td class="text-xs-left">
                        <v-btn color="success" round @click="imprimirSeguimiento(props.item.registro_id,'historia')">Imprimir Historia</v-btn>
                        </td>
                        <td>
                            <v-btn depressed small round color="success" @click="cargarDetalles(props.item)">Generar
                                Seguimiento</v-btn>
                        </td>
                        <td>
                ...
                <v-dialog v-model="dialogDetalle" persistent max-width="1900px">
                    <v-card>
                        ...
                        <v-card-text>
                            <v-form @submit.prevent="guardarSeguimientos">
                                <v-container>
                                    <v-layout row wrap>
                                        <!-- pregunta 2 -->
                                        ...
                                        <!-- pregunta 3 -->
                                        <v-flex xs12 sm4 d-flex v-if="formSeguimiento.contacto_fue == 'EFECTIVO'">
                                            <v-select :items="ambitoatencion" label="¿ÁMBITO DE LA ATENCIÓN*"
                                                v-model="formSeguimiento.actualizar_info_hospitalizaciones_paciente">
                                            </v-select>
                                        </v-flex>
                                        <!-- pregunta 5 -->
                                        <v-flex xs12 sm3 d-flex
                                            v-if="formSeguimiento.actualizar_info_hospitalizaciones_paciente == 'HOSPITALIZACIÓN'">
                                        ...
                                        <!-- pregunta 12 -->
                                        <v-flex xs12 sm3 d-flex v-if="
                                            formSeguimiento.se_encuentra_hospitalizado == 'NO'
                                            || formSeguimiento.se_encuentra_hospitalizado == 'SI'
                                            || formSeguimiento.actualizar_info_hospitalizaciones_paciente == 'HOSPITALIZACIÓN'
                                            || formSeguimiento.actualizar_info_hospitalizaciones_paciente == 'DOMICILIARIO'
                                            || formSeguimiento.actualizar_info_hospitalizaciones_paciente == 'AMBULATORIO'
                                            || formSeguimiento.actualizar_info_hospitalizaciones_paciente == 'OTRO'
                                            ">
                                        ...
                                        <!-- campo nuevo -->
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field label="Saturación" type="number"
                                                v-model="formSeguimiento.saturacion">
                                            </v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field label="Pulso" type="number" v-model="formSeguimiento.pulso">
                                            </v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field label="Temperatura" v-model="formSeguimiento.temperatura">
                                            </v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field label="Tensión Arterial"
                                                v-model="formSeguimiento.tensionarterial">
                                            </v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field label="Frecuencia Respiratoria"
                                                v-model="formSeguimiento.frecuenciarespiratoria">
                                            </v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="escalanews2" label="Escala News 2"
                                                v-model="formSeguimiento.escalanews2">
                                            </v-select>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-text-field
                                                label="Call Score"
                                                type="number"
                                                v-model="formSeguimiento.callscore">
                                            </v-text-field>
                                        </v-flex>
                                        <!-- campo nuevo -->
                                        <!-- pregunta 13 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-select :items="sino" label="Fiebre cuantificada ≥ 38°C"
                                                v-model="formSeguimiento.ha_presentado_fiebre">
                                            </v-select>
                                        </v-flex>
                                        <!-- pregunta 14 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-select :items="sino" label="Tos"
                                                v-model="formSeguimiento.ha_presentado_tos"></v-select>
                                        </v-flex>
                                        <!-- pregunta 15 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-select :items="sino" label="Dificultad respiratoria"
                                                v-model="formSeguimiento.ha_presentado_dificultad_respiratoria">
                                            </v-select>
                                        </v-flex>
                                        <!-- pregunta 16 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-select :items="sino" label="Dolor de garganta"
                                                v-model="formSeguimiento.ha_presentado_dolor_garganta">
                                            </v-select>
                                        </v-flex>
                                        <!-- pregunta 17 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-select :items="sino" label="Fatiga"
                                        ...
                                        <!-- pregunta 19 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                            <v-text-field label="Qué otros síntomas ha presentado"
                                                v-model="formSeguimiento.que_otros_sintomas">
                                            </v-text-field>
                                        </v-flex>
                                        <!-- pregunta 20 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.paciente_sintomas == 'SI'">
                                        ...
                                        <v-container fluid v-if="formSeguimiento.ha_presentado_sintomas_alarma == 'SI'">
                                        ...
                                        <!-- pregunta 22 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.ha_presentado_sintomas_alarma == 'SI' || formSeguimiento.paciente_sintomas == 'NO'
                                            || formSeguimiento.paciente_sintomas == 'SI' || formSeguimiento.contacto_fue == 'FALLIDO PORQUE NO CONTESTA'
                                             || formSeguimiento.contacto_fue == 'FALLIDO PORQUE NO SE ENCUENTRA EN EL LUGAR DE RESIDENCIA'  || formSeguimiento.contacto_fue == 'FALLIDO POR TELÉFONO ERRADO'
                                              || formSeguimiento.contacto_fue == 'FALLIDO POR OTRAS CAUSAS'">
                                            <v-select :items="sino" label="Se encuentra registrado en SEGCOVID"
                                                v-model="formSeguimiento.registrado_segcovid"></v-select>
                                        </v-flex>
                                        <!-- pregunta 23 -->
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.ha_presentado_sintomas_alarma == 'SI'
                                        || formSeguimiento.paciente_sintomas == 'NO'
                                        || formSeguimiento.paciente_sintomas == 'SI' || formSeguimiento.contacto_fue == 'FALLIDO PORQUE NO CONTESTA'
                                             || formSeguimiento.contacto_fue == 'FALLIDO PORQUE NO SE ENCUENTRA EN EL LUGAR DE RESIDENCIA'  || formSeguimiento.contacto_fue == 'FALLIDO POR TELÉFONO ERRADO'
                                              || formSeguimiento.contacto_fue == 'FALLIDO POR OTRAS CAUSAS'">
                                            <v-select :items="sino"
                                        ...
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="causaseguimiento" label="CAUSA DEL SEGUIMIENTO"
                                                v-model="formSeguimiento.causaseguimiento"></v-select>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="tiposeguimiento" label="TIPO DE SEGUIMIENTO"
                                                v-model="formSeguimiento.tiposeguimiento"></v-select>
                                        </v-flex>
                                        <!-- campo nuevo -->
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="sino" label="Se Toma de laboratorios a Domicilio"
                                                v-model="formSeguimiento.laboratoriodomicilio">
                                            </v-select>
                                        </v-flex>
                                        <!-- campo nuevosss -->
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="sino" label="Oxigenoterapia"
                                                v-model="formSeguimiento.oxigenoterapia">
                                            </v-select>
                                        </v-flex>
                                        <v-flex xs12 sm3 d-flex v-if="formSeguimiento.oxigenoterapia == 'SI'">
                                            <v-select :items="oxigeno" label="Flujo de oxígeno indicado"
                                                v-model="formSeguimiento.flujo_oxigeno">
                                            </v-select>
                                        </v-flex>
                                        <!-- pregunta 33 -->
                                        ...
                                        <v-flex xs12 sm3 d-flex>
                                            <v-select :items="control" label="Control"
                                                v-model="formSeguimiento.control">
                                            </v-select>
                                        </v-flex>
                                        <!-- ¿INDICACIONES? -->
                                        ...
                                        <v-container fluid>
                                            <v-layout row wrap>
                                                <v-flex xs12 sm4 md3>
                                                    ...
                                                    <v-checkbox v-model="formSeguimiento.consultaprioritaria"
                                                        label="Se Requiere Consulta Prioritaria" color="indigo"
                                                        value="x" hide-details>
                                                    </v-checkbox>
                                                </v-flex>
                                                <v-flex xs12 sm4 md3>
                                                    <v-checkbox v-model="formSeguimiento.teleorientacion"
                                                        label="Teleorientación Salud Mental" color="orange" value="x"
                                                        hide-details>
                                                    </v-checkbox>
                                                    <v-checkbox v-model="formSeguimiento.otrasindicaciones"
                                                        label="Otros" color="green" value="x" hide-details>
                                                    </v-checkbox>
                                                </v-flex>
                                                <v-flex xs12 sm4 md3>
                                                    <v-checkbox v-model="formSeguimiento.concentracionoxigeno"
                                                        label="Concentrador de Oxígeno" color="pink" value="x"
                                                        hide-details>
                                                    </v-checkbox>
                                                </v-flex>
                                            </v-layout>
                                        ...
                                        <v-card>
                                                <v-layout row wrap>
                                                    ...
                                                    <v-flex xs12 sm3 d-flex
                                                        ...
                                                        <v-text-field readonly
                                                            label="Fecha de Ingreso a la hospitalización"
                                                            v-model="item.fecha_ingreso_hospitalizacion" type="date">
                                                        </v-text-field>
                                                    </v-flex>
                                                    <v-flex xs12 sm3 d-flex
                                                        v-if="item.se_encuentra_hospitalizado != null">
                                                        <v-select readonly :items="sino"
                                                    ...
                                                    <v-flex xs12 sm3 d-flex
                                                        ...
                                                        <v-text-field readonly
                                                            label="Fecha de egreso a la hospitalización"
                                                    ...
                                                    <v-flex xs12 sm3 d-flex>
                                                        <v-text-field label="Saruración" type="number"
                                                            v-model="item.saturacion">
                                                        </v-text-field>
                                                    </v-flex>
                                                    <v-flex xs12 sm3 d-flex>
                                                        <v-text-field label="Pulso" type="number" v-model="item.pulso">
                                                        </v-text-field>
                                                    </v-flex>
                                                    <v-flex xs12 sm3 d-flex>
                                                        <v-text-field label="Temperatura" v-model="item.temperatura">
                                                        </v-text-field>
                                                    </v-flex>
                                                    <v-flex xs12 sm3 d-flex>
                                                        <v-text-field label="Tensión Arterial"
                                                        ...
                                                        <v-layout row wrap>
                                                            <v-flex xs12 sm4 md3>
                                                                <v-checkbox v-if="item.detalles_signos_alarmas != 0"
                                                                    ...
                                                                    label="detalles signos alarmas" color="red"
                                                                    hide-details readonly>
                                                                </v-checkbox>
                                                                <v-checkbox
                                                                    v-if="item.respiracion_rapida_taquipnea != 0"
                                                                ...
                                                                <v-checkbox v-if="item.fiebre_mas_2_dias != 0"
                                                                    ...
                                                                    label="fiebre mas 2 dias" color="indigo"
                                                                    hide-details readonly>
                                                                </v-checkbox>
                                                            ...
                                                            <v-flex xs12 sm4 md3>
                                                                <v-checkbox v-if="item.somnolencia_letargia != 0"
                                                                    ...
                                                                    label="somnolencia letargia" color="orange"
                                                                    hide-details readonly>
                                                                </v-checkbox>
                                                                ...
                                                                <v-checkbox
                                                                    v-if="item.deterioro_rapido_estado_general != 0"
                                                                    v-model="item.deterioro_rapido_estado_general"
                                                                    label="deterioro rapido estado general"
                                                                    color="primary" hide-details readonly></v-checkbox>
                                                            </v-flex>
                                                        </v-layout>
                                                    </v-container>
                                                    ...
                                                    <v-flex xs12 sm3 d-flex
                                                        v-if="item.pruebas_diagnostico_covid != null">
                                                        <v-select readonly :items="sino"
                                                            label="Le han realizado pruebas para Diagnóstico de Covid"
                                                    ...
                                                    <v-flex xs12 sm3 d-flex
                                                        v-if="item.fecha_realizacion_prueba != null">
                                                        <v-text-field readonly
                                                    ...
                                                    <v-flex xs12 sm3 d-flex v-if="item.ips_toma_muestra != null">
                                                        <v-select readonly :items="ipsprueba"
                                                            label="IPS que toma la muestra"
                                                            v-model="item.ips_toma_muestra"></v-select>
                                                    </v-flex>
                                                    ...
                                                    <v-flex xs12 sm3 d-flex v-if="item.otra_ips != null">
                                                        <v-text-field readonly label="Cual IPS?"
                                                            v-model="item.otra_ips">
                                                        </v-text-field>
                                                    </v-flex>
                                                    ...
                                                    <v-flex xs12 sm3 d-flex>
                                                        <v-select :items="causaseguimiento"
                                                            label="CAUSA DEL SEGUIMIENTO"
                                                        ...
                                                        <v-layout row wrap>
                                                            <v-flex xs12 sm4 md3>
                                                                ...
                                                                <v-checkbox v-model="item.seguimientotelefonico" readonly
                                                                    label="Se Realizará Seguimiento Telefónico Para Confirmación De Estado De Salud Por Parte Del Personal Médico Del Contratista"
                                                                    color="blue" value="x" hide-details readonly>
                                                                </v-checkbox>
                                                                <v-checkbox v-model="item.consultaprioritaria"
                                                                    label="Se Requiere Consulta Prioritaria"
                                                                    color="indigo" value="x" hide-details readonly>
                                                                </v-checkbox>
                                                            </v-flex>
                                                            <v-flex xs12 sm4 md3>
                                                                ...
                                                                <v-checkbox v-model="item.otrasindicaciones"
                                                                    label="Otros" readonly color="green" value="x" hide-details>
                                                                </v-checkbox>
                                                                <v-checkbox v-model="item.concentracionoxigeno"
                                                                    label="Concentrador de Oxígeno" readonly color="pink" value="x"
                                                                    hide-details>
                                                                </v-checkbox>
                                                            </v-flex>
                                                        </v-layout>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                control:['1 hora', '2 horas','4 horas','24 horas','48 horas','72 horas'],
                oxigeno: ['2 lt min','3 lt min','4 lt min','5 lt min'],
                ...
                ambitoatencion: ['HOSPITALIZACIÓN', 'DOMICILIARIO', 'AMBULATORIO', 'OTRO'],
                ...
                headers: [{
                    ...
                    {
                        text: "Destino admisiones"
                    },
                    {
                        text: "Destino seguimiento"
                    },
                    {
                        text: "Ficha",
                        value: ""
        ...
        methods: {
            ...
            guardarSeguimientos() {
                ...
                if (this.formSeguimiento.oxigenoterapia == null) {
                    this.$alerError('Seleccione oxigenoterapia.')
                    return;
                }
                axios.post('/api/covid/guardarSeguimientos', this.formSeguimiento).then(res => {
                    ...
                    this.$router.push(
                        "/covi/seguimiento?cita_paciente=" + this.formSeguimiento.cita_paciente_id
                    );
            ...
            listarSolicitudes() {
                axios.get('/api/covid/listarregistros').then(res => {
                    this.listaSolicitudes = res.data;

                })
            },
            cargarDetalles(items) {
                this.formSeguimiento.registrocovi_id = items.registro_id;
                this.formSeguimiento.cita_paciente_id = items.cita_paciente_id;
