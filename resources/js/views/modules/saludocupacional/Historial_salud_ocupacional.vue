    ...
    <v-layout>
        <v-flex xs12>
            <v-card>
                        ...
                        <v-flex xs12 md12 lg12>
                            <v-card>
                                <v-card-title class="headline success" style="color:white">
                                    <span class="title layout justify-center font-weight-light">Historias clinicas</span>
                                </v-card-title>
                                <v-container>
                                    <v-layout row wrap>
                                        <v-flex xs6 sm12 md6>
                                        ...
                                        <v-btn color="success" round @click="cargue()"
                                            >Cargar historias</v-btn>
                                    </v-card-actions>
                                         </v-flex>
                                    </v-layout>
                                </v-container>
                ...
                <v-layout row>
                    <v-flex xs12 md12 lg12>
                        <v-data-table :headers="header" :items="historiaOcupacional" item-key="index"
                            v-if="dialog_timeline">
                            <template v-slot:items="props">
                                <td class="text-xs-center">{{ props.item.Num_Doc}}</td>
                                <td class="text-xs-center">{{ props.item.Primer_Nom}} {{ props.item.SegundoNom}} {{ props.item.Primer_Ape}} {{ props.item.Segundo_Ape}}</td>
                                <td class="text-xs-center">{{ props.item.Edad_Cumplida}}</td>
                                <td class="text-xs-center">{{ props.item.Sexo}}</td>
                                <td class="text-xs-center">{{ props.item.Datetimeingreso}}</td>
                                <td class="text-xs-center">{{ props.item.salud_ocupacional}}</td>
                                    ...
                                    <v-tooltip top>
                                        <template v-slot:activator="{ on }">
                                            <v-btn fab outline color="primary" small v-on="on"
                                                @click="consultarAdjunto(props.item.ruta)">
        ...
        <v-dialog v-model="dialogoCargue" persistent max-width="850">
            <v-card>
                <CargueSaludOcupacionalComponetVue />
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="error" dark @click="dialogoCargue = false">Cerrar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-layout>
...
import CargueSaludOcupacionalComponetVue from '../../../components/cargueSaludOcupacional/CargueSaludOcupacionalComponet.vue';
    export default {
        components: {
            CargueSaludOcupacionalComponetVue
        },
        data: () => ({
            ...
            header: [
                ...
                {text: 'Fecha atencion', value: 'Datetimeingreso'},
                {text: 'Tipo Cita', value: 'salud_ocupacional'},
                {text: 'Generar historiaOcupacional'}
        ...
        methods: {
            ...
            printhc(id) {
                    let pdf = {type:"historiasst", id:id};
                    axios
            ...
            find() {
                ...
                axios
                    .post("/api/cita/historiaOcupacional", {Num_Doc: this.cedula})
                    .then(res => {
                        console.log('tabla', res.data);
                        console.log('ss', this.historiaOcupacional.Correo1);
                        this.preload_historico = false;
                        this.historiaOcupacional = res.data;
