...
class CertificadoEmpleadosActivos extends FPDF
{
    ...
    {
        $pdf = new CertificadoEmpleadosActivos('p', 'mm', 'A4');
        self::$empleado = gestion_humana_certificados::select('*')
            ->where('documento', $cedula1)
            ->where('estado_empleado', 'ACTIVO')
            ->OrderBy('fecha_de_aumento')
            ->first();
        $pdf->AliasNbPages();
        $pdf->AddPage();
    ...
    public function body($pdf)
    {
        ...
        $mes = Carbon::parse(self::$empleado->fecha_ingreso)->isoFormat('D MMMM YYYY');
        $dia = Carbon::now()->isoFormat('D [dias del mes de] MMMM [de] YYYY');

        $salario = number_format(self::$empleado->salario);
        $cedula = number_format(self::$empleado->documento, 0, ",", ".");

        $f = new NumberFormatter("es", NumberFormatter::SPELLOUT);
        $letra = $f->format(self::$empleado->salario);

        $pdf->SetXY(25, 80);
        $pdf->SetFont('Arial', '', 12);
        $pdf->MultiCell(155, 5, utf8_decode(self::$empleado->nombre_del_completo . ',' . ' identificado(a) con cédula de ciudadanía número' . ' ' .
            $cedula . ',' . ' ' . 'se encuentra laborando en esta empresa como' . ' ' . self::$empleado->cargo_empleado . ' ' .
            'desde el día ' . $mes . ' hasta la fecha' . ';' . ' con un contrato a término fijo, con un salario básico mensual de ' . '$' . $salario . ' ' . '(' . $letra . ')' . '.'), 0, 'J');
        $pdf->Ln();
