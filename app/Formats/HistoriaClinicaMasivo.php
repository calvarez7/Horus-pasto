...
class HistoriaClinicaMasivo extends FPDF
{
    public static $data;
    ...
    public function generar($datos)
     {
         self::$data = $datos;
        $pdf = new HistoriaClinicaMasivo('P','mm','A4');
        $pdf->AliasNbPages();
        $pdf->AddPage();

         foreach ($datos as  $key => $obj){
            $dato = json_decode(json_encode($obj), true);
            self::$data = $dato;
            if($dato['Tipocita_id'] == '8' || $dato['Tipocita_id'] == '2' || $dato['Tipocita_id'] == '10' || $dato['Tipocita_id'] == '4'){
                $this->bodyHistoriaClinica($pdf);
                if(isset($datos[$key+1])){
                    $pdf->AddPage();
                }
            }elseif($dato['Tipocita_id'] == '12'){
                $this->bodyOftalmologia($pdf);
                if(isset($datos[$key+1])){
                    $pdf->AddPage();
                }
            }elseif($dato['Tipocita_id'] == '7'){
            $this->bodyHistoriaClinicaOncologia($pdf);
                if(isset($datos[$key+1])){
                    $pdf->AddPage();
                }
        }elseif ($dato['Tipocita_id'] == '6'){
...
//         if(!file_exists (public_path().'/CC'.$datos[0]->Cédula.'/')){
//             mkdir (public_path().'/CC'.$datos[0]->Cédula.'/');
//         }
//        $pdf->Output('F',public_path()."/CC".$datos[0]->Cédula.'/'.$datos[0]->Cédula."_HISTORIAS.pdf" , true);
        $pdf->Output();
    ...
    public function bodyOftalmologia($pdf){
        ...
        $pdf->SetXY(12,$posicionFamiliares+38);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->MultiCell(62,8,'BIOMICROSCOPIA',1,'C');
        $pdf->SetXY(74,$posicionFamiliares+38);
        $pdf->Cell(30, 4, utf8_decode('OJO DERECHO'), 1, 0, 'C');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(94, 4, utf8_decode(self::$data['biomicros_ojoderecho']), 1, 0, 'C');
        $pdf->Ln();
        $pdf->SetX(74);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(30, 4, utf8_decode('OJO IZQUIERDO'), 1, 0, 'C');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(94, 4, utf8_decode(self::$data['biomicros_ojoizquierdo']), 1, 0, 'C');

        $pdf->SetXY(12,$posicionFamiliares+50);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->MultiCell(30,8,'PRESION INTRAOCULAR',1,'C');
        $pdf->SetXY(42,$posicionFamiliares+50);
        $pdf->MultiCell(20,4,'OJO DERECHO',1,'C');
        $pdf->SetXY(42,$posicionFamiliares+58);
        $pdf->MultiCell(20,4,'OJO IZQUIERDO',1,'C');
        $pdf->SetXY(62,$posicionFamiliares+50);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(25, 8, utf8_decode(self::$data['presionintra_ojoderecho']), 1, 'C');
        $pdf->SetXY(62,$posicionFamiliares+58);
        $pdf->MultiCell(25, 8, utf8_decode(self::$data['presionintra_ojoizquierdo']), 1, 'C');

        $pdf->SetFont('Arial', 'B', 9);
        $pdf->SetXY(100,$posicionFamiliares+50);
        $pdf->MultiCell(30,16,'GINIOSCOPIA',1,'C');
        $pdf->SetXY(130,$posicionFamiliares+50);
        $pdf->MultiCell(30,8,'OJO DERECHO',1,'C');
        $pdf->SetXY(130,$posicionFamiliares+58);
        $pdf->MultiCell(30,8,'OJO IZQUIERDO',1,'C');
        $pdf->SetXY(160,$posicionFamiliares+50);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(38,8,utf8_decode(self::$data['gionios_ojoderecho']),1,'C');
        $pdf->SetXY(160,$posicionFamiliares+58);
        $pdf->MultiCell(38,8,utf8_decode(self::$data['gionios_ojoizquierdo']),1,'C');


        $pdf->SetXY(12,$posicionFamiliares+70);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->MultiCell(50,8,'FONDO DE OJO',1,'C');
        $pdf->SetXY(62,$posicionFamiliares+70);
        $pdf->Cell(30, 4, utf8_decode('OJO DERECHO'), 1, 0, 'C');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(106, 4, utf8_decode(self::$data['fondo_ojoderecho']), 1, 0, 'L');
        $pdf->SetXY(62,$posicionFamiliares+74);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(30, 4, utf8_decode('OJO IZQUIERDO'), 1, 0, 'C');
        $pdf->SetFont('Arial', '', 8);
        ...
        $pdf->SetXY(62,$posicionFamiliares+82);
        $pdf->SetFont('Arial', '', 8);
        $count3 = 0;
        if($diagnosticos){
        ...
        $pdf->SetXY(12,$posicionFamiliares+82);
        $pdf->SetFont('Arial', 'B', 9);
        if($count3 > 4){
...
//$pdf->Image($logo, 0, 291, -95);
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Oftalmologico"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('GENITOURINARIO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Genitourinario"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(40, 4, utf8_decode('OTORRINOLARINGÓLOGO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(146, 4, utf8_decode(self::$data["Otorrinoralingologico"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('LINFÁTICO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Linfatico"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(35, 4, utf8_decode('OSTEOMIOARTICULAR:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(151, 4, utf8_decode(self::$data["Osteomioarticular"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('NEUROLÓGICO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Neurologico"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('CARDIOVASCULAR:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Cardiovascular"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('TEGUMENTARIO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Tegumentario"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('RESPIRATORIO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(self::$data["Respiratorio"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(32, 4, utf8_decode('ENDOCRINOLÓGICO:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(154, 4, utf8_decode(self::$data["Endocrinologico"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(32, 4, utf8_decode('GASTROINTESTINAL:'), 0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(154, 4, utf8_decode(self::$data["Gastrointestinal"]), 0, 0, 'L', 0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 4, utf8_decode('OTROS:'),0, 0, 'L', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(156, 4, utf8_decode(''), 0, 0, 'L', 0);
        $pdf->Ln();
        ...
        $pdf->SetFillColor(214, 214, 214);
        $pdf->SetTextColor(0, 0, 0);
        $antecedentesPersonales = Pacienteantecedente::select('pacienteantecedentes.created_at AS fecha', DB::raw("CONCAT(u.name,' ',u.apellido) AS medico"), 'a.Nombre AS antecedente', 'pacienteantecedentes.Descripcion AS descripcion')
            ->join('antecedentes as a', 'a.id', 'pacienteantecedentes.Antecedente_id')
            ...
            ->where('p.Num_Doc', self::$data["Cédula"])->get();
        ...
        $pdf->SetFillColor(214, 214, 214);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(46.5, 4, utf8_decode('FECHA'), 1, 0, 'C', 1);
        $pdf->Cell(46.5, 4, utf8_decode('MÉDICO'), 1, 0, 'C', 1);
        $pdf->Cell(46.5, 4, utf8_decode('ANTECEDENTE'), 1, 0, 'C', 1);
        $pdf->Cell(46.5, 4, utf8_decode('DESCRIPCIÓN'), 1, 0, 'C', 1);
        $pdf->Ln();
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetDrawColor(0, 0, 0);
        $pdf->SetX(12);
        ...
        foreach ($antecedentesPersonales as $antedentes) {
            ...
            $pdf->Cell(46.5, 4, utf8_decode($antedentes->antecedente), 0, 0, 'L', 0);
        ...
        $antecedentesParentesto = Parentescoantecedente::select('parentescoantecedentes.created_at AS fecha', DB::raw("CONCAT(u.name,' ',u.apellido) AS medico"), 'a.Nombre AS antecedente', 'parentescoantecedentes.Descripcion AS descripcion')
            ->join('antecedentes as a', 'a.id', 'parentescoantecedentes.Antecedente_id')
            ->join('cita_paciente as c', 'c.id', 'parentescoantecedentes.Citapaciente_id')
            ->join('pacientes as p', 'p.id', 'c.Paciente_id')
        ...
        foreach ($antecedentesParentesto as $antedentes) {
            $pdf->Cell(46.5, 4, utf8_decode(substr($antedentes->fecha, 0, 10)), 1, 0, 'L', 0);
            $pdf->Cell(46.5, 4, utf8_decode($antedentes->medico), 1, 0, 'L', 0);
            $pdf->Cell(46.5, 4, utf8_decode($antedentes->antecedente), 1, 0, 'L', 0);
            $pdf->Cell(46.5, 4, utf8_decode($antedentes->descripcion), 1, 0, 'L', 0);
            $pdf->Ln();
            $pdf->SetX(12);
        }
        ...
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(46.5, 4, utf8_decode('¿TIENE DIETA SALUDABLE?'), 1, 0, 'L',0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(139.5, 4, utf8_decode(self::$data["Dietasaludable"]), 1, 0, 'L',0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(46.5, 4, utf8_decode('¿TIENE SUEÑO REPARADOR?'), 1, 0, 'L',0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(139.5, 4, utf8_decode(self::$data["Suenoreparador"]), 1, 0, 'L',0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(46.5, 4, utf8_decode('¿DUERME MENOS DE 6 HORAS?'), 1, 0, 'L',0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(139.5, 4, utf8_decode(self::$data["Duermemenoseishoras"]), 1, 0, 'L',0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(46.5, 4, utf8_decode('¿ALTO NIVEL DE ESTRES?'), 1, 0, 'L',0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(139.5, 4, utf8_decode(self::$data["Altonivelestres"]), 1, 0, 'L',0);
        $pdf->Ln();
        $pdf->SetX(12);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->SetDrawColor(0,0,0);
        $pdf->SetFillColor(214, 214, 214);
        $pdf->SetTextColor(0,0,0);
        $pdf->Cell(186, 4, utf8_decode('¿REALIZA ACTIVIDAD FÍSICA?'), 1, 0, 'C',1);
        $pdf->Ln();
        $pdf->SetTextColor(0,0,0);
        $pdf->SetDrawColor(0,0,0);
        $pdf->SetX(12);
