...
class USCovidRepository{
    ...
    public function consultar($data) {
        if($data->sede === 0){
            $consulta = collect(DB::select("SET NOCOUNT ON exec dbo.SP_RIPS_COVID_US_CONGLOMERADO ?,?,?,?", [$data->fecha_inicial, $data->fecha_final, $data->entidad, 0]));
        }else{
            $consulta = collect(DB::select("SET NOCOUNT ON exec dbo.SP_RIPS_COVID_US ?,?,?,?", [$data->fecha_inicial, $data->fecha_final, $data->entidad, $data->sede]));
        }
