...
class F2BRepository implements FiasInterface{
    ...
    public function consultar($data){
        ...
        $select = [
            'AH_RIPS_FIAS.dane_dpto as codigo_departamento',
            'AH_RIPS_FIAS.dane_mpio as codigo_municipio',
            'AH_RIPS_FIAS.diagnostico_principal_egreso as codigo_cie10',
            DB::raw('count (diagnostico_principal_egreso) total')
        ];
        ...
        $consulta = Ahripsfias::select($select)
            ...
            ->groupBy('AH_RIPS_FIAS.dane_dpto', 'AH_RIPS_FIAS.dane_mpio', 'AH_RIPS_FIAS.diagnostico_principal_egreso')
            ->orderBy('total', 'desc')
            ->get();
        ...
        foreach($departamentos as $departamento){
            ...
            foreach($municipios as $municipio){
                ...
                foreach ($cie10s as $cie10) {
                    $pacientes = Ahripsfias::select($select_pacientes)
                        ->join('pacientes', 'AH_RIPS_FIAS.num_doc', '=', 'pacientes.Num_Doc')
                        ->whereFecha($data['mes'], $data['anio'])
                        ->where('AH_RIPS_FIAS.dane_dpto', $departamento)
                        ->where('AH_RIPS_FIAS.dane_mpio', $municipio)
                        ->where('AH_RIPS_FIAS.diagnostico_principal_egreso', $cie10)
                        ->where('AH_RIPS_FIAS.diagnostico_principal_egreso', 'not like', 'z%')
                        ->get();
