...
class ATRepository{
    ...
    public function consultar($data) {
        $consulta = collect(DB::select("SET NOCOUNT ON exec dbo.SP_RIPS_AT ?,?,?,?", [$data->fecha_inicial, $data->fecha_final, $data->entidad, $data->sede]));
        return $consulta->map(function ($item){
            return [
                ...
                'valor_unitario' => round($item->valor_unitario),
                'valor_total' => round($item->valor_total),
            ];
        });
    }
