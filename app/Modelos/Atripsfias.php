...
class Atripsfias extends Model implements Auditable
{
    use HasFactory;
    ...
    public function scopeWhereDepartamento($query, $departamento_id){
        if($departamento_id){
            $query->where('AT_RIPS_FIAS.dane_dpto', $departamento_id);
        }
    }

    public function scopeWhereAmbito($query, $ambito){
        return $query->whereIn('AT_RIPS_FIAS.cod_cup', $ambito);
    }
}
